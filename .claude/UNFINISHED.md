...
## 2026-07-30-0 (갱신) — 대부분 완료, 남은 것만 정리

### 완료된 것 (이번 QA 세션)
- Febucci NRE는 블로커가 아니었음 — Stop→Play 재진입으로 매번 해소됨(다만 이번 세션은 재현 빈도가 예전보다 잦았다, `design-issues.md` 2026-07-30-0 "검증 못한 부분" 참고).
- **Pierce 카드 통합 완전 검증 완료** — 드래프트 풀 1장만 존재, 스택 누적(+=) 정상, CentralTower 전용 게이팅 확인. [CardManager.md](class/CardManager.md) 2026-07-30-5 참고.
- **난이도 사다리 전수 조사 완료** — Normal 신규 유저 기준 7회 연속 시도(메타 트리 0→5개 순차 재투자), 전부 실패(최고 324초/목표 600초). Hard/Hell은 참고 데이터만(강제 해금 상태에서 30초 안팎 즉사). 상세는 [design-issues.md](qa/design-issues.md) 2026-07-30-0 참고 — **헤드라인 결론: Normal 자체가 자연 클리어 불가능한 벽, Hard/Hell/Infinite는 사실상 도달 불가능**.
- 신규 무기 중 Mortar(#8)는 실제 드래프트에서 뽑혀 정상 동작 확인(에러 0건, 무기 수 정상 증가). Frost Orb Turret(#7)은 이번 세션 드래프트에 안 떠서 미확인.
- 무기 슬롯 4개 제한은 여러 런에서 weaponCount가 정확히 4에서 plateau — 확인됨.

### 아직 안 한 것
- Frost Orb Turret(#7) 개별 동작(공전/슬로우/글로우) 실측 — 메타 트리 M-205 해금 후 카드가 실제로 뜨는지부터 확인 필요.
- InGameHUD 레벨 표시/무기 쿨다운 ScrollView/카메라 자동 줌아웃/Pause 중 BGM 유지 — 시각적 확인이 필요해 이번 세션(수치 중심 QA)에서 다루지 않음.
- Orbital Ring 비주얼(오브 5개, 반경, 색 Tween 주기) — 시각 확인 필요, 미착수.
- **"카드 선택 시 더블샷처럼 발사체 2배" 미해결 버그** — 이번에도 재현 시도 안 함(다른 조사에 집중). `UICheatWindow` 카드 즉시 적용으로 이진 탐색 권장.
- Normal을 실제로 클리어하는 데 필요한 메타 투자량 확정 안 됨 — 별도 세션에서 대량 반복 또는 메타 트리 최대 해금 상태로 재확인 필요.
