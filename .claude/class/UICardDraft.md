...
## 개요
UICardDraft.prefab 루트에 부착되는 화면 컴포넌트 — 현재는 **빈 껍데기(UIBase 상속만)**. UIManager.Get<UICardDraft>(경로)로 접근하기 위한 타입. 실제 표시/갱신 로직은 추후 구현.

## 현재 상태
- `public class UICardDraft : UIBase { }` (멤버 없음)
