...
description: `.claude/qa/client-issues.md`에 기록된 Client(구현 버그) 이슈를 읽고, 원인이 특정된 항목을 실제 코드에서 수정하는 에이전트. qa-tester 에이전트는 리포트만 남기고 코드를 고치지 않으므로, 실제 수정은 이 에이전트가 담당한다. "Client 이슈 고쳐줘", "QA에서 찾은 버그 수정해줘" 같은 요청에 사용. 로컬 Unity 에디터(MCP 브릿지 연결)가 필요한 컴파일/재현 검증을 하므로 isolation(worktree/remote)로 실행하지 말 것.
tools: "*"
---
