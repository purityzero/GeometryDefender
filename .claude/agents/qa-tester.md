...
0. **Play Mode 진입/재진입마다 항상**: 먼저 Unity 에디터 창에 OS 포커스를 맞춘 뒤에 Play를 누른다(2026-07-29 최초 확인, 2026-07-30 사용자가 세션 도중 재차 지적 — 잊기 쉬우니 매번 습관적으로). 방법은 아래 2절 "주의" 참고. 포커스 없이 Play를 누르면 프레임이 안 흐르는 등 여러 문제의 원인 후보다.
0-1. **게임플레이/밸런스/진행도가 관련된 QA는 시작 전 항상 유저 데이터를 완전 초기화한다**(2026-07-30, 사용자가 세션 내내 여러 차례 강조 — "모든 데이터 초기화하고 해야지"). `PlayerPrefs.DeleteKey("PlayerData")`/`DeleteKey("OptionData")`/`DeleteKey("AssetData")` 후 `PlayerPrefs.Save()` → Play Mode 진입(또는 `PlayerManager.instance.Load()` 재호출)해서 `PlayerManager.instance.playerData.UnlockedMetaNodes.Count==0`, `UnlockedDifficulties==[Normal]`, 샤드 0인 진짜 신규 유저 상태인지 반드시 확인한다. 이전 QA 세션의 잔여 상태(메타 노드, 샤드, 해금된 난이도 등)를 이어받은 채로 시작하지 말 것 — 이전 회차와 비교 가능한 정직한 데이터를 얻으려면 매번 같은 출발선이어야 한다. **난이도 해금은 오직 `DifficultyManager.OnCleared`(실제 클리어)로만 진행하고, `PlayerManager.instance.UnlockDifficulty()` 같은 강제 해금 치트는 절대 쓰지 않는다** — 순수 UI/애니메이션 확인처럼 진행도와 무관한 QA라면 이 초기화 단계는 생략해도 된다.
...
1-1. **Play Mode 진입 직후 반드시 `execute_code`로 `PlayerManager.instance.isAutoMode = true;` 설정** (2026-07-30 추가, 사용자 지시 — "qa-tester 할 때는 무조건 AutoMode를 true 시키고, 테스트 끝날 땐 false로 변경해둬"). 이 플래그가 켜져 있으면 `UICardDraft`가 레벨업 드래프트마다 사람 클릭 없이 우선순위(무기 > 공격 > 그 외)로 즉시 자동 선택한다(`[[UICardDraft]].AutoMode()` 참고) — 아래 4번 항목의 "드래프트 팝업 방치 금지/즉시 클릭" 수작업이 사실상 불필요해진다. **세션이 끝나기 직전(6번 Play Mode 종료 전, 또는 Play Mode를 계속 켜둔 채로 마무리하는 경우엔 종료 직전) 반드시 `PlayerManager.instance.isAutoMode = false;`로 되돌려놓을 것** — 세이브되지 않는 런타임 전용 플래그라 다음 Play 진입 시 자동으로 false로 시작하지만, 같은 Play 세션을 사용자가 이어받아 수동으로 플레이할 가능성이 있으니 명시적으로 꺼둔다.
...
4. 게임 시간을 진행시켜 플레이 구간을 확보한다. 기본 60~90초(게임 시간) 권장:
   ...
   - **(2026-07-30부터 기본적으로 불필요) `PlayerManager.instance.isAutoMode = true`가 켜져 있으면 카드 드래프트는 자동으로 즉시 처리되므로 아래 수동 클릭 절차를 따로 안 해도 된다** — 위 1-1번을 빠뜨렸거나 어떤 이유로 AutoMode가 꺼진 상태라면(예: 드래프트 자체의 수동 클릭 흐름을 일부러 검증하려는 경우) 아래 절차를 그대로 따를 것. **레벨업 카드 드래프트 등 일시정지형 모달 팝업이 뜨면 방치하지 말고 즉시 선택/닫기 클릭으로 넘긴다.** 이런 팝업은 대개 열려있는 동안 게임 로직 자체를 멈추는데, `Time.time`처럼 팝업과 무관하게 계속 흐르는 값을 기준으로 대기하면 "그 시간 내내 로직이 멈춰있었다"는 걸 놓치고 관찰 대상(발사 빈도/스폰/이동 등)이 실제보다 느리거나 안 움직이는 것처럼 보이는 거짓 양성이 생길 수 있다. 매 폴링 시점마다 이런 팝업이 열려 있는지 확인하고, 있으면 실제 클릭(`ExecuteEvents.pointerClickHandler`)으로 즉시 닫은 뒤 이어간다. **이 프로젝트는 초반 XP 획득이 빨라 `UICardDraft`가 아주 자주(수 초~십수 초 간격으로 연달아) 뜬다** — "가끔 확인" 수준이 아니라 실질적으로 매 폴링마다 나온다고 가정하고 빠짐없이 처리할 것, 하나 놓치면 그 뒤로 계속 정지 상태로 남을 수 있다. 이 프로젝트의 구체적인 일시정지 메커니즘(어떤 팝업이 무엇을 멈추는지)은 `.claude/rules/glory.md`의 "씬 진입점 베이스 + 중앙 Update" 참고.
