...
public class ProjectileManager : SceneSingleton<ProjectileManager>, IUpdatable
{
    private const int POOL_SIZE = 20;

    // 프리팹 스프라이트(shape_circle)의 스케일 1일 때 지름 — ProjectileRecord.Size(반지름)로부터 시각 스케일을 역산하는 데 사용
    private const float PREFAB_NATIVE_DIAMETER = 2.22f;
    ...
    public void Init()
    {
        ...
        m_ProjectileFactory = new MemoryPoolFactory<ActorProjectile, eProjectileType>(pathMap, POOL_SIZE, m_PoolParent);
    ...
    /// <summary>타워가 호출 — _projectileId는 ProjectileTable의 Id(TowerRecord.ProjectileId로 지정). 크기/색/프리팹/관통 등은 전부 테이블에서 읽는다.</summary>
    public void Fire(Vector2 _from, Vector2 _to, int _baseDamage, float _speed, float _range, int _projectileId)
    {
    ...
    private void SpawnVisual(Entity _entity, ProjectileRecord _record)
    {
        ...
        float visualScale = (_record.Size * 2f) / PREFAB_NATIVE_DIAMETER;
